      ...
      <section className="mx-auto flex w-full max-w-6xl flex-col gap-8">
        <header className="rounded-3xl border border-slate-200/70 bg-white/85 p-6 shadow-[0_18px_50px_-28px_rgba(15,23,42,0.45)] backdrop-blur md:p-8">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-700">Swagger Frontend</p>
              <h1 className="mt-2 text-3xl font-bold tracking-tight md:text-4xl">
                {spec?.info?.title || "Event Manager API"}
              </h1>
              <p className="mt-2 text-sm text-slate-600">Version: {spec?.info?.version || "unknown"}</p>
            </div>
          ...
          {spec?.info?.description ? (
            <p className="mt-4 max-w-3xl text-sm leading-relaxed text-slate-600">{spec.info.description}</p>
          ) : null}
        </header>

        {!spec ? (
          <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900">
            <h2 className="text-base font-semibold">Backend OpenAPI unavailable</h2>
            <p className="mt-2 text-sm">
              Could not fetch <strong>{`${backendBaseUrl}/openapi.json`}</strong>. Start the backend container/server and refresh.
            </p>
          </section>
        ) : null}
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white/90 p-4">
            ...
            <p className="mt-1 text-2xl font-semibold">{new Set(endpoints.map((e) => e.tag)).size}</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white/90 p-4">
            <p className="text-xs uppercase tracking-wide text-slate-500">Auth Domains</p>
            ...
            {endpoints.map((endpoint) => (
              ...
              >
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="rounded bg-slate-100 px-2 py-1 text-xs font-medium text-slate-700">{endpoint.tag}</span>
                  <p className="font-mono text-sm font-semibold text-slate-800">{endpoint.path}</p>
                </div>
                <p className="mt-2 text-sm font-medium text-slate-800">{endpoint.summary}</p>
                {endpoint.description ? <p className="mt-1 text-sm text-slate-600">{endpoint.description}</p> : null}
              </article>
            ))}
